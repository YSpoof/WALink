<script lang="ts">
  ...
  const formatInput = (value: string) => {
    ...
    if (digits.length >= 3) {
      // Decide split based on length
      const isMobile = digits.length === 11;

      const firstPart = isMobile
        ? digits.slice(2, 7) // 5 digits
        : digits.slice(2, 6); // 4 digits
    ...
    if (digits.length >= 7) {
      ...
      const secondPart = isMobile
        ? digits.slice(7) // last 4
        : digits.slice(6); // last 4
...
<main class="min-h-screen bg-gray-50 py-12 px-6 flex items-start justify-center">
  <div class="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-10">
      ...
      <form onsubmit={generateLink}>
        <div class="mt-6">
          ...
          <div class="mt-2 flex items-center gap-3">
            ...
            <input
              bind:value={() => number, (v) => (number = formatInput(v ?? ""))}
              autofocus
    ...
    <section class="flex items-center justify-center">
      <div class="w-full max-w-xs bg-white border rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="h-96 phone-safe-area p-4">
          <div class="h-full rounded-md bg-gray-50 border border-dashed border-gray-200"></div>
        </div>
    ...
    >
      <div class="p-6">
        <p class="text-gray-800 font-medium">Seu link foi gerado com sucesso!</p>
        <input
          type="text"
          value={fullLink}
          readonly
        ...
        <div class="mt-4 flex justify-between gap-3">
          ...
          <a
            href={fullLink}
            target="_blank"
            class="uppercase w-full bg-blue-500 text-white px-4 py-2 rounded-md">Testar</a
          >
          <button
